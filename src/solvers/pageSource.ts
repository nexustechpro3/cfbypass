...
import { withCtx, setupPage, waitForCF, buildProxyUrl } from './base'
...
export async function getPageSource(req: BypassRequest): Promise<{ source: string; solveMs: number; proxy: string | null }> {
  ...
  return withCtx(req.proxy, async ctx => {
    ...
    const sourcePromise = new Promise<string>((res, rej) => {
      resolveSource = res
      rejectSource = rej
    })
    ...
    page.on('response', async response => {
      ...
      if ([200, 301, 302].includes(status) && [req.url, req.url + '/'].includes(resUrl)) {
        try {
          ...
          if (!resolved) {
            resolved = true
            rejectSource(err instanceof Error ? err : new Error(String(err)))
          }
        }
      }
    })
