...
import { withCtx, setupPage, waitForToken, buildProxyUrl } from './base'
...
export async function solveTurnstileMin(req: BypassRequest): Promise<{ token: string; solveMs: number; proxy: string | null }> {
  ...
  return withCtx(req.proxy, async ctx => {
    ...
    // Intercept navigation to target URL — respond with injected fake page
    await page.route(
    ...
    // Poll for Turnstile token — loads in isolation, no CF challenge needed
